...
#ifdef __AVX2__
template < typename T >
void sw_parallel_alpern_256(std::vector<std::pair< T, T >> const sequences, unsigned int const quantity, unsigned int const size){
    // SIMD size
    ...
    std::vector<__m256i> max_element( num_threads );
    std::vector<__m256i> max_element_i( num_threads );
    std::vector<__m256i> max_element_j( num_threads );

    // Auxiliary values
    std::vector<__m256i> diagonal_value( num_threads );
    std::vector<__m256i> top_value( num_threads );
    std::vector<__m256i> left_value( num_threads );
    std::vector<__m256i> temp_value( num_threads );
    std::vector<__m256i> target_value( num_threads );
    std::vector<__m256i> i_vectorized( num_threads );
    std::vector<__m256i> j_vectorized( num_threads );
    std::vector<__m256i> max_element_updated( num_threads );
    std::vector<__m256i> mask( num_threads );
    std::vector<__m256i> match_val( num_threads );

    // Char batching containers
    std::vector<std::vector<__m256i>> i_seq( num_threads, std::vector<__m256i>( size ) );
    std::vector<std::vector<__m256i>> j_seq( num_threads, std::vector<__m256i>( size ) );
    int char_batch_i[ num_threads ][ sse_s ];
    int char_batch_j[ num_threads ][ sse_s ];
    ...
    for (unsigned int k = 0; k < quantity; k += sse_s) {
        ...
        // Set target values
        max_element[t]   = _mm256_setzero_si256();
        max_element_i[t] = _mm256_setzero_si256();
        max_element_j[t] = _mm256_setzero_si256();

        // Construct next SIMDed batch of chars
        for (unsigned int i = 0; i < size; ++i) {
            for (unsigned int p = 0; p < sse_s; ++p) {
                char_batch_i[t][p] = int(sequences[k + p].first[i]);
                char_batch_j[t][p] = int(sequences[k + p].second[i]);
            }
            i_seq[t][i] = _mm256_load_si256( ( __m256i * ) char_batch_i );
            j_seq[t][i] = _mm256_load_si256( ( __m256i * ) char_batch_j );
        }
        
        for (unsigned int i = 1; i < size + 1; ++i){
            i_vectorized[t] = _mm256_set1_epi32( i );
            for(unsigned int j = 1; j < size + 1; ++j) {
                j_vectorized[t] = _mm256_set1_epi32( j );
                // match_val ~ data.first[i - 1] == data.second[j - 1] ? match : mismatch)
                mask[t]      = _mm256_cmpeq_epi32( i_seq[t][i - 1], j_seq[t][j - 1] );
                match_val[t] = _mm256_blendv_epi8( mismatch, match, mask[t] );
                
                // diagonal_value ~ matrix[i-1][j-1] + match_val
                diagonal_value[t]   = _mm256_add_epi32 ( matrices[t][i-1][j - 1], match_val[t] );
                // top_value ~ matrix[i-1][j] + gap
                top_value[t]        = _mm256_add_epi32 ( matrices[t][i-1][j], gap );
                // left_value ~ matrix[i][j-1] + gap
                left_value[t]       = _mm256_add_epi32 ( matrices[t][i][j - 1], gap);
                
                // Calculate target_value ~ std::max(diagonal_value, std::max(top_value, left_value))
                temp_value[t]    = _mm256_max_epi32( top_value[t], left_value[t] );
                target_value[t]  = _mm256_max_epi32( diagonal_value[t], temp_value[t] );
                // Calculate  matrix[i][j] ~ (target_value > 0) ? target_value : 0
                matrices[t][i][j]  = _mm256_max_epi32( target_value[t], _mm256_setzero_si256() );
                // Update max_element and coordinates if the target_value is larger
                max_element[t]         = _mm256_max_epi32( max_element[t], target_value[t] );
                max_element_updated[t] = _mm256_cmpeq_epi32( max_element[t], target_value[t] );
                max_element_i[t]       = _mm256_blendv_epi8( max_element_i[t], i_vectorized[t], max_element_updated[t] );
                max_element_j[t]       = _mm256_blendv_epi8( max_element_j[t], j_vectorized[t], max_element_updated[t] );
            }
        }
        if (k % (1u << 9) == 0) {
            auto const vec = reinterpret_cast< int const * >( &max_element[t] );
...
template < typename T >
    void sw_parallel_alpern(std::vector<std::pair< T, T >> const sequences){
        ...
        sw_parallel_alpern_256(sequences, quantity, size);
        #else

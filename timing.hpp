...
template < typename Callable, typename Container >
	duration benchmark( Callable f, Container test_instances )
	{
		...
		// compiler is set to full optimization so compiler recognizes that we never produce any output, when running benchmarks we have to be careful about the compiler
